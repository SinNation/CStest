...
for (
    ...
    list_of_files
):  # Uses the list of files to allow us to cycle through all the files in the project
    file_path = (
        project_path + "\\" + file
    )  # Constructs the file path for the specific file
    try:
        with open(file_path, encoding="utf-8") as cs_file:
            code = (
                cs_file.readlines()
            )  # Reads each line of the file in as a list of strings
    except:
        print(
            "******************************************************************************************\
              \nCStest encountered an error when trying to open file: "
            + file
    ...
    try:
        ...
        ]  # Remove the generated line breaks in the code

        error_location = "uppercase strings."
        uppercase = [
            line_of_code.upper() for line_of_code in remove_breaks
        ]  # Uppercase all the strings

        error_location = "generate the numbered code."
        numbered_code = (
            []
        )  # Generate a list to store all the code strings alongside the row number
        for row_number, line_of_code in enumerate(
            uppercase
        ):  # Loop through all the lines of code in the file
            if str.isspace(
                line_of_code
            ):  # If the string contains only spaces then do nothing
                pass
            else:
                if (
                    not line_of_code
                ):  # Or if the string is completely empty then do nothing
                    pass
                else:
                    numbered_code.append(
                        [row_number + 1, line_of_code]
                    )  # Otherwise append it to our list with the row number
    except:
        print(
            "******************************************************************************************\
              \nCStest encountered an error when trying to "
            + error_location
            + "\
              \nThe error occured with file: "
            + file
            + "\
    ...
    complete_code[
        file
    ] = numbered_code  # Add the code for each file as a list of strings to the dictionary. The key is the file name
##################################################################################################################
...
print(
    str(number_of_files)
    + ".txt files have been found in project folder: "
    + project_folder
    + "\n"
)
print("Outcome of tests will be stored in: " + test_run_path + "\n")
print(
    "Finished creating the code for all "
    + str(number_of_files)
...
list_of_variables = (
    []
)  # List to contain all the created variables, to be populated into the dictionary
created_variables = (
    {}
)  # Dictionary to hold all the variables created in the game and the file in which they are created
list_of_duplicate_variables = (
    []
)  # Variables that are created more than once in the startup file
duplicate_variables = {}


for original_string in complete_code[
    "startup.txt"
]:  # Take all the lines of code in the startup file. This gives us a list for each string (the row number and the code)
    string = original_string[1]  # So take just the code and pass that in as our string
    if (
        "*CREATE" in string
    ):  # If there is a *create command in our string, then it denotes the creation of a variable
        variable = string[string.find("*CREATE") + len("*CREATE") :].split()[
            0
        ]  # Split that string to just take the first word after the create command (chopping off the command and the initial value
        if (
            variable in list_of_variables
        ):  # If the resulting variable already exists in our list, don't add it and instead identify it as a duplicate
            list_of_duplicate_variables.append([original_string[0], variable])
        else:
            list_of_variables.append(
                [original_string[0], variable]
            )  # Append to our list, taking the row number from the original string

    if (
        "*TEMP" in string
    ):  # If there is a *temp command in our string, then it denotes the creation of a variable
        variable = string[string.find("*TEMP") + len("*TEMP") :].split()[
            0
        ]  # Split that string to just take the first word after the temp command (chopping off the command and the initial value
        if (
            variable in list_of_variables
        ):  # If the resulting variable already exists in our list, don't add it and instead identify it as a duplicate
            list_of_duplicate_variables.append([original_string[0], variable])
...
created_variables[
    "startup.txt"
] = list_of_variables  # Add the list of variables as a value in the dictionary with the key being the startup.txt file name

if list_of_duplicate_variables:
    duplicate_variables[
        "startup.txt"
    ] = list_of_duplicate_variables  # Add the list of duplicate variables as a value in the dictionary with the key being the startup.txt file name
...
for (
    ...
    complete_code.items()
):  # Now loop through all the files to look for temp variables in the remainder of the code. We want to bypass the startup file as we have already handled it
    if file != "startup.txt":  # Only process the file if it isn't the startup
        list_of_variables = (
            []
        )  # Reset the list of variables, so it is calculated fresh for each file
        list_of_duplicate_variables = (
            []
        )  # Reset the list of duplicate variables, so it is calculated fresh for each file
        for original_string in code:  # Follow the same pattern as above
            ...
            if "*TEMP" in string:
                ...
                # The difference here is that we want to append to the dictionary at the end of each file, so that we store the variables by file
                    ...
                    list_of_variables.append(
                        [original_string[0], variable]
                    )  # Create a list of all variable names in the file which follow the *temp command
        created_variables[
            file
        ] = list_of_variables  # Add the list of variables as a value in the dictionary with the key being the file name
...
print(
    "Identified all created and temp variable names in codebase\
       \n"
)
##################################################################################################################
...
def find_variables_in_command_strings(
    variables_in_file, string, bracket_variables_in_file, row_number
):
    words_to_remove = (
        []
    )  # A list we are going to populate to hold all the strings that are not variables and need removing
    variables_to_add = (
        []
    )  # A list of words we need to add, from parsing certain variable names. We remove the original and add this version instead
    multi_word_string = (
        "false"  # Control within a loop to identify strings containing multiple words
    )
    bracket_variables_in_string = (
        []
    )  # A list to hold all the variables utilising [] in their names to be handled separately

    strings_to_remove_from_commands = [
        "ROUND(",
        "MODULO",
        "LENGTH(",
        "(",
        ...
        "£",
        "NOT(",
    ]  # All valid characters that need to be cleaned out of strings
    for string_to_remove in strings_to_remove_from_commands:
        string = string.replace(
            string_to_remove, " "
        )  # Remove the characters so that we are, as best as possible, just left with variable names.

    # Split the remaining string down into individual words. We need to evaluate each word independently now.
    variables_in_string = (
        string.split()
    )  # This is a our baseline for all the variables in the string - we will remove/add as required
    ...
    try:
        for (
            ...
            variables_in_string
        ):  # Loop through each word and check if it meets any criteria which denotes it as not being a variable
            error_location = ""

            if (
                multi_word_string == "true"
            ):  # If a previous word was identified as the start of a multi-word string, then this kicks in and keeps flagging the words until it finds the end of the string
                error_location = "Looking for end of multi-word string"
                if word.endswith("'") or word.endswith(
                    '"'
                ):  # If the word is the end of the string
            ...
            if (
                ...
                or word.startswith('"')
                and word.endswith('"')
            ):  # If the word is contained in quotes, i.e. is a value and not an attribute name
                error_location = "Checking actual strings"
                words_to_remove.append(word)

            if word.startswith("'") or word.startswith(
                '"'
            ):  # If the word is the start of a string of words (so it has opening quotes) - triggers the loop above
            ...
            ):  # In some cases, # is used to pull out the numbered character from a string, this chops off the # and number, just leaving the variable name
            ...
            if (
                "[" in word
            ):  #'Bracket variables' are cases where a variable name is constructed by referring to another variable value. This identifies these.
                error_location = "Removing [] words"
                bracket_variables_in_string.append(
                    word
                )  # We document the full variable as one to be handled separately
                words_to_remove.append(
                    word
                )  # So we remove it from our list as we will handle it separately
                word = word.split("[")[
                    1
                ]  # We can process the variable name in the brackets though, so split off everything after it
                word = word.replace("]", "")  # Replace the closing bracket
                if (
                    "#" in word
                ):  # If that variable also has a # in it to denote a character number, then split that off also
                    word = word.split("#")[0]
    ...
    for variable in bracket_variables_in_string:
        if any(
            variable == file_variable[1] for file_variable in bracket_variables_in_file
        ):
            pass
        else:
...
def find_variables_in_prose_strings(
    variables_in_file, string, bracket_variables_in_file, row_number
):
    strings_to_remove_from_prose = [
        "ROUND(",
        "MODULO",
        "LENGTH(",
        "(",
        ...
        "*page_break",
        "*line_break",
    ]  # Slightly different characters to strip from the string
    for string_to_remove in strings_to_remove_from_prose:
        string = string.replace(
            string_to_remove, " "
        )  # Loop through the characters and replace them.
    if (
        "{" in string
    ):  # In prose, variables are called with {} - this identifies that the string contains a variable
        ...
        for word in words_in_string:  # Loop through each word
            if "{" in word:  # The word is a variable call
                word = word.split("}")[
                    0
                ]  # Split out anything before or after the {} (including the {} )
                word = word.split("{")[1]
                if (
                    "[" in word
                ):  # If the variable contains a [ then handle it in the same way as a command variable
                    if any(
                        word == file_variable[1]
                        for file_variable in bracket_variables_in_file
                    ):  # Don't do anything if the variable is already added for that file
                        pass
                    else:
                        bracket_variables_in_file.append(
                            [row_number, word]
                        )  # Store it as a bracket variable for handling separately
                    word = word.split("[")[1]  # Strip off the square brackets
                    word = word.replace("]", "")
                    if "#" in word:
                        word = word.split("#")[0]  # Strip off the # if there is one
                    if any(
                        word == file_variable[1] for file_variable in variables_in_file
                    ):  # Leaving us with the variable called in the brackets - first check if we have already added it for the file
                        pass
                    else:
                    ...
                    if any(
                        word == file_variable[1] for file_variable in variables_in_file
                    ):  # Or it had no [] and is just a variable call. Check it isn't alreay added for the file
                        pass
                    else:
                        variables_in_file.append([row_number, word])  # Then add it
    ...
    )  # Return the two lists of variables in the file
...
for (
    ...
    complete_code.items()
):  # Gives us the entire list of lists containing all the code in each file
    variables_in_file = (
        []
    )  # Creates our master list to store all the variables called in the file. We will pass this in and out of the functions
    bracket_variables_in_file = (
        []
    )  # Creates our master list to store all the bracket variables called in the file. We will pass this in and out of the functions
    ...
    for original_string in code:
        string = original_string[
            1
        ]  # Redefine our string as just the line of code, ignoring the row number

        # Remove AND, OR, TRUE and FALSE as there are frequently found in the code but are not variable names
        string = re.sub(
            r"\bAND\b", "", string
        )  # Remove 'and' where it is a standalone word and not part of a longer word
        string = re.sub(
            r"\bOR\b", "", string
        )  # Remove 'or' where it is a standalone word and not part of a longer word
        string = re.sub(
            r"\bTRUE\b", "", string
        )  # Remove 'true' where it is a standalone word and not part of a longer word
        string = re.sub(
            r"\bFALSE\b", "", string
        )  # Remove 'false' where it is a standalone word and not part of a longer word
        string = re.sub(
            r"\bNOT\b", "", string
        )  # Remove 'not' where it is a standalone word and not part of a longer word
        ...
        if (
            ...
            or string.startswith("*SET")
            or string.startswith("*ELSEIF")
            or string.startswith("*ELSE")
            or string.startswith("*INPUT_TEXT")
            or string.startswith("*INPUT_NUMBER")
        ):
            (
                variables_in_file,
                bracket_variables_in_file,
            ) = find_variables_in_command_strings(
                variables_in_file, string, bracket_variables_in_file, original_string[0]
            )  # Pass in the cleansed string and the original row number
        ...
        # If the command is a '*selectable_if' string, then split the string and pass the command half and prose half to the functions respectively
            ...
            command_string = string.split("#")[
                0
            ]  # Split out before the #, giving us just the code
            prose_string = string.replace(
                command_string, ""
            )  # Then the remainder is the prose - cut off the command string
            ...
            (
                variables_in_file,
                bracket_variables_in_file,
            ) = find_variables_in_prose_strings(
                variables_in_file,
                prose_string,
                bracket_variables_in_file,
                original_string[0],
...
for (
    created_file,
    created_variable_row,
) in created_variables.items():  # Loop through all the files
    variables_not_called_in_file = []
    for (
        created_variable
    ) in created_variable_row:  # Loop through all the variables created in the file
        ...
        for (
            ...
            called_variables.items()
        ):  # Loop through all the files to find if the variable is called in it
            if (
                created_file == "startup.txt" or called_file == created_file
            ):  # Only compare variables defined in the startup (which can be called anywhere), or variables defined in the SAME file we're checking (for temp variables)
                if variable_found == False:  # If we haven't already found it
                    for (
                        called_variable
                    ) in (
                        called_variable_row
                    ):  # Loop through all the called variables in the file
                        if variable_found == False:  # If we haven't already found it
                            if (
                                created_variable[1] == called_variable[1]
                            ):  # And the variable we are currently comparing matches the one we are searching for. Here we take the second element of the list, as the first element is the row number
                                variable_found = True  # Then set it to 'found'

        for (
            called_bracket_file,
            called_bracket_variable_row,
        ) in called_bracket_variables.items():  # Repeat for the bracket variables
                ...
                if variable_found == False:  # If we haven't already found it
                    for called_bracket_variable in called_bracket_variable_row:
                        if variable_found == False:
                            if (
                                created_variable[1].split("_")[0]
                                == called_bracket_variable[1].split("[")[0]
                            ):  # Difference here is we split the variables before the _ and brackets and simply match on the stub rather than the full variable name
                                variable_found == True

        if (
            variable_found == False
        ):  # After looking for the variable in every file and not finding it, add it as a non-called variable
            variables_not_called_in_file.append(
                [created_variable[1], created_variable[0]]
            )
...
for (
    called_file,
    called_variable_row,
) in called_variables.items():  # Loop through our files
    variables_not_defined_in_project = []
    variables_called_before_defined_in_project = []
    for (
        called_variable
    ) in called_variable_row:  # Loop through each called variable in the file
        if (
            called_variable[1] not in variables_defined_in_startup
        ):  # If the variable name is in our list (generated in these loops), then we already know it has been defined and can be skipped
            variable_defined = (
                False  # Set variable as not defined, until we see that it is
            )

            for (
                created_file,
                created_variable_row,
            ) in created_variables.items():  # Loop through each file
                if (
                    created_file == "startup.txt" or called_file == created_file
                ):  # If the file is startup or the same file we are checking, then proceed (we don't want to check temp variables in other files)
                    if variable_defined == False:  # If we haven't already found it
                        for (
                            created_variable
                        ) in (
                            created_variable_row
                        ):  # Loop through each created variable in that file
                            if (
                                variable_defined == False
                            ):  # If we haven't se the variable to defined yet
                                if (
                                    called_variable[1] == created_variable[1]
                                ):  # Then check if the variable we are checking matches the one we are searching for
                                    variable_defined = (
                                        True  # And if so, set it to 'defined'
                                    )
                                    row_defined_on = 0
                                    if created_file != "startup.txt":
                                        row_defined_on = created_variable[
                                            0
                                        ]  # Record the row number to now check which row it was defined on
                                    if created_file == "startup.txt":
                                        variables_defined_in_startup.append(
                                            called_variable[1]
                                        )  # We query this list so that if we encounter the same variable in a different file, we already know it has been defined

            if (
                variable_defined == False
            ):  # If we get through all the files and we never find the variable, add it as a non-defined variable
                variables_not_defined_in_project.append(
                    [called_variable[1], called_variable[0]]
                )
            else:
                if (
                    called_variable[0] <= row_defined_on
                ):  # Otherwise check what row it was called on and if it was defined on an earlier row, add it to the list
                    variables_called_before_defined_in_project.append(
                        [called_variable[1], called_variable[0], row_defined_on]
                    )
...
for (
    called_bracket_file,
    called_bracket_variable_row,
) in called_bracket_variables.items():
    ...
    for called_bracket_variable in called_bracket_variable_row:
        ...
        if variable_defined == False:
            bracket_variables_not_defined_in_project.append(
                [
                    called_bracket_file,
                    called_bracket_variable[1],
                    called_bracket_variable[0],
                ]
            )
        else:
            if (
                called_bracket_variable[0] <= row_defined_on
            ):  # Otherwise check what row it was called on and if it was defined on an earlier row, add it to the list
                bracket_variables_called_before_defined_in_project.append(
                    [
                        called_bracket_variable[1],
                        called_bracket_variable[0],
                        row_defined_on,
...
for file, code in complete_code.items():  # Loop through each file
    ...
    for original_string in code:  # Loops through each string
        ...
        no_space_string = (
            string.lstrip()
        )  # Strip the indent so we can check what the string actually starts with

        actual_indent = len(string) - len(
            string.lstrip(" ")
        )  # The indent of the string
        ...
        for count in range(active_choices):
            if in_choice == True:  # If currently evaluating a choice block
                if (
                    actual_indent <= choice_indents[active_choices - 1]
                ):  # And the indent of this line is less than or equal to the indent of the choice command
                    choice_indents.pop(
                        active_choices - 1
                    )  # Then this choice is finished, remove its indent
                    option_indents.pop(
                        active_choices - 1
                    )  # And remove the option indent
                    ...
                    if (
                        active_choices == 0
                    ):  # If there are now no active choices, it is no longer evaluating a choice
                        in_choice = False
        ...
        if in_option == True:
            ...
            if (
                (
                    no_space_string.startswith("#")
                    or no_space_string.startswith("*SELECTABLE_IF")
                )
                or (
                    no_space_string.startswith("*ALLOW_REUSE")
                    and "#" in no_space_string
                )
                or (
                    no_space_string.startswith("*DISABLE_REUSE")
                    and "#" in no_space_string
                )
                or (
                    no_space_string.startswith("*HIDE_REUSE") and "#" in no_space_string
                )
            ):
                in_option = False
            ...
            if active_choices == 1:
                if (
                    no_space_string.startswith("*IF")
                    and actual_indent > choice_indents[active_choices - 1]
                    and actual_indent <= option_indents[active_choices - 1]
                ):
                    in_option = False
                    previous_prose = False  # When we exit an option block, the requirements to follow prose with the same indent do not apply, so we can set this to false
            ...
            if active_choices > 1 and len(option_indents) < active_choices:
                if (
                    no_space_string.startswith("*IF")
                    and actual_indent > choice_indents[active_choices - 2]
                    and actual_indent <= option_indents[active_choices - 2]
                ):
                    in_option = False
                    previous_prose = False  # When we exit an option block, the requirements to follow prose with the same indent do not apply, so we can set this to false
        ...
        if no_space_string.startswith("*CHOICE") or no_space_string.startswith(
            "*FAKE_CHOICE"
        ):
            in_choice = True
            active_choices += 1
        ...
        # If the current string is an option, then set in_option and option_line (the line that initiates the option). Then adds the indent of the option to the list
            ...
            no_space_string.startswith("#")
            or no_space_string.startswith("*SELECTABLE_IF")
            or (no_space_string.startswith("*ALLOW_REUSE") and "#" in no_space_string)
            or (no_space_string.startswith("*DISABLE_REUSE") and "#" in no_space_string)
            or (no_space_string.startswith("*HIDE_REUSE") and "#" in no_space_string)
        ):
            in_option = True
            option_line = True
            if (
                len(option_indents) == active_choices
            ):  # Each option in a choice must have the same or lower indent than the previous option. If this is the first option for the choice
                option_indents[
                    active_choices - 1
                ] = actual_indent  # Then add it to the list of option indents
            ...
            if (
                first_option == False
            ):  # If first option is false, then this must be the first option, so set it
                first_option = True
        ...
        if (
            no_space_string.startswith("*")
            or option_line == True
            or no_space_string.startswith("OPPOSED_PAIR")
        ):
            if (
                "*COMMENT" in no_space_string or "*RETURN" in no_space_string
            ):  # If it is a comment or return, then it can ignore indent rules
                comment_return_line = True
            elif (
                "*IF" in no_space_string
                or "*ELSEIF" in no_space_string
                or "*ELSE" in no_space_string
                or "*STAT_CHART" in no_space_string
                or "OPPOSED_PAIR" in no_space_string
                or "*ACHIEVEMENT" in no_space_string
            ):  # If it is an IF command, then we are now evaluating an IF command
                in_if = True
                active_ifs += 1
        ...
        if option_line == True:
            if (
                actual_indent <= choice_indents[active_choices - 1]
                and comment_return_line == False
            ):
                file_invalid_indents.append(
                    [
                        string,
                        original_string[0],
                        "Options (#) within a choice block must have a greater indent than the *choice command",
                        choice_indents[active_choices - 1],
                        actual_indent,
                    ]
                )
            ...
            if (
                actual_indent > option_indents[active_choices - 1]
                and first_option == False
                and comment_return_line == False
            ):
                file_invalid_indents.append(
                    [
                        string,
                        original_string[0],
                        "Options (#) within a choice block can not have a greater indent that previous options",
                        option_indents[active_choices - 1],
                        actual_indent,
                    ]
                )
        ...
        if in_option == True and option_line == False and comment_return_line == False:
            ...
            if (
                no_space_string.startswith("*CHOICE")
                or no_space_string.startswith("*FAKE_CHOICE")
            ) or (
                no_space_string.startswith("*IF")
                and active_choices > len(option_indents)
            ):
                if actual_indent <= option_indents[active_choices - 2]:
                    file_invalid_indents.append(
                        [
                            string,
                            original_string[0],
                            "The contents of an option (#) must have a greater indent than the option command",
                            option_indents[active_choices - 2],
                            actual_indent,
                        ]
                    )
        ...
        if (
            ...
            and (
                ...
                and "*ELSEIF" not in no_space_string
                and "*ELSE" not in no_space_string
                and "*STAT_CHART" not in no_space_string
                and "OPPOSED_PAIR" not in no_space_string
                and "*ACHIEVEMENT" not in no_space_string
            )
            and actual_indent <= if_indents[active_ifs - 1]
        ):
            file_invalid_indents.append(
                [
                    string,
                    original_string[0],
                    "The contents of an if statement must have a greater indent than the if command",
                    if_indents[active_ifs - 1],
                    actual_indent,
                ]
            )
        ...
        if previous_prose == True:
            if (
                actual_indent != previous_indent
                and option_line == False
                and comment_return_line == False
            ):
                file_invalid_indents.append(
                    [
                        string,
                        original_string[0],
                        "Prose and command lines following prose must have the same indent",
                        previous_indent,
                        actual_indent,
                    ]
                )
...
for file, duplicate_variables in duplicate_variables.items():
    for duplicate_variable in duplicate_variables:
        variable_output.append(
            ["duplicate variable", file, duplicate_variable[0], duplicate_variable[1]]
        )
    ...
    for variable_not_called in variables_not_called:
        variable_output.append(
            [
                "variable not called",
                file,
                variable_not_called[0],
                variable_not_called[1],
            ]
        )
    ...
    for bracket_variable_not_defined in bracket_variables_not_defined:
        variable_output.append(
            [
                "variable not defined",
                file,
                bracket_variable_not_defined[0],
                bracket_variable_not_defined[1],
            ]
        )
...
for file, variables_called_before_defined in variables_called_before_defined.items():
    for variable_called_before_defined in variables_called_before_defined:
        variable_output.append(
            [
                "variable called before defined",
                file,
                variable_called_before_defined[0],
                variable_called_before_defined[1],
            ]
        )
...
for (
    file,
    bracket_variables_called_before_defined,
) in bracket_variables_called_before_defined.items():
    for (
        bracket_variable_called_before_defined
    ) in bracket_variables_called_before_defined:
        variable_output.append(
            [
                "variable called before defined",
                file,
                bracket_variable_called_before_defined[0],
                bracket_variable_called_before_defined[1],
            ]
        )

output_df = pd.DataFrame(
    variable_output, columns=["test", "filename", "variable_name", "row_number"]
)
